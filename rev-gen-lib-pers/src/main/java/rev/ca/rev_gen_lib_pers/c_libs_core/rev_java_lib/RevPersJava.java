...
import rev.ca.rev_gen_lib_pers.rev_pers_services.rev_pers_action.rev_pre_post_pers.RevPluginsPostActionsRegistry;
import rev.ca.rev_gen_lib_pers.rev_pers_services.rev_pers_action.rev_pre_post_pers.RevPluginsPreActionsRegistry;
...
public class RevPersJava {
    ...
    public Object saveRevEntity(final Object revObject) {
        ...
        if (revObject.getClass() == RevEntity.class) {
            ...
            for (IPreRevPersAction iPreRevPersAction : new RevPluginsPreActionsRegistry().initRevPluginsPreActionsRegistry()) {
                iPreRevPersAction.initPrePersRevAction(revEntity);
            }

            revPersId = revPersLibCreate.revPersInit(revEntity);
            revEntity.set_revEntityGUID(revPersId);

            for (IPostRevPersAction iPostRevPersAction : new RevPluginsPostActionsRegistry().initRevPluginsPostActionsRegistry()) {
                iPostRevPersAction.initPostPersRevAction(revEntity);
            }
        ...
        if (revPersId > 0) {
            if (RevCheckConnectivity.isIsRevNetworkConnected()) {
                new RevSaveRevEntitiesToServerResolver(RevLibGenConstantine.REV_CONTEXT).syncNewRevEntities();
            }
        }
