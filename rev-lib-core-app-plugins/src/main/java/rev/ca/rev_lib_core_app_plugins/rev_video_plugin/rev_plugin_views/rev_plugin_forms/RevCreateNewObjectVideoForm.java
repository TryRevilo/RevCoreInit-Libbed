package rev.ca.rev_lib_core_app_plugins.rev_video_plugin.rev_plugin_views.rev_plugin_forms;
...
import rev.ca.rev_lib_core_app_plugins.rev_file_plugin.rev_lib_file_functions.RevVideoFileCrawler;
...
public class RevCreateNewObjectVideoForm implements IRevInputFormView, IRevFileCrawler {
    ...
    public RevCreateNewObjectVideoForm(RevVarArgsData revVarArgsData) {
    ...
    public RevEntity createRevInputFormData() {
        ...
        revObjectEntitySuper.set_revEntityType(FeedReaderContract.FeedEntry_REV_OBJECT_ENTITY.TABLE_NAME);
        revObjectEntitySuper.set_revEntitySubType("rev_video");
        revObjectEntitySuper.set_revEntityOwnerGUID(REV_SESSION_SETTINGS.getRevLoggedInEntityGuid());
        ...
        for (String imageURL : selectedItems) {
            revEntityMetadataList.add(new RevEntityMetadata("video_url", imageURL));
        }

        revObjectEntitySuper.set_revEntityMetadataList(revEntityMetadataList);
        ...
        submitFormTab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                RevConstantinePluggableViewsServices
                        .REV_PLUGIN_START_REV_PERS_ACTIONS_MAP.get("RevPublishPublishVideoAction").initRevAction(createRevInputFormData());
            }
        });
    ...
    public LinearLayout genPublisherWrapper_LL() {
        ...
        textView.setText("Select Videos");
        textView.setTextColor(mContext.getResources().getColor(rev.ca.rev_lib_core_views.R.color.teal_dark));
        textView.setGravity(Gravity.CENTER_VERTICAL);
        textView.setPadding(RevLibGenConstantine.REV_MARGIN_TINY, RevLibGenConstantine.REV_MARGIN_TINY, RevLibGenConstantine.REV_MARGIN_MEDIUM, RevLibGenConstantine.REV_MARGIN_TINY);
        ...
        final RevCreateNewObjectVideoForm revPublisher = this;
            ...
            public void onClick(View v) {
                if (selectedItems != null) {
                    ...
                    for (String path : selectedItems)
                        revVarArgsData.getRevVarArgsDataMetadataStrings().put(path, "video_path");
                }

                final RevVideoFileCrawler revVideoFileCrawler = new RevVideoFileCrawler(revVarArgsData, revPublisher);
                RevPop.initiatePopupWindow_MATCH_WIDTH(revVideoFileCrawler.createRevVideoFileCrawler());
            }
        });
        ...
        for (int i = 0; i < selectedItems.size(); i++) {
            ...
            if (new File(imgPath).exists()) {
                ...
                if (new File(imgPath).exists()) {
                    ...
                    Bitmap thumb = ThumbnailUtils.createVideoThumbnail(imgPath, MediaStore.Images.Thumbnails.MINI_KIND);
                    final BitmapDrawable bitmapDrawable = new BitmapDrawable(thumb);

                    int w = thumb.getWidth();
                    int h = thumb.getHeight();

                    imageView.setLayoutParams(new LinearLayout.LayoutParams(h, w));
