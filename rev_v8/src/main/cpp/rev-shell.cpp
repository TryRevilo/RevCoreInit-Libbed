...
bool ExecuteString(v8::Isolate *isolate, v8::Local<v8::String> source, v8::Local<v8::Value> name,
    ...
    if (!v8::Script::Compile(context, source, &origin).ToLocal(&script)) {
        ...
        if (!script->Run(context).ToLocal(&result)) {
            __android_log_print(ANDROID_LOG_ERROR, "MyApp", ">>>>>>>>>>>>>>>>>>>>>ERR Run");
            assert(try_catch.HasCaught());
            // Print errors that happened during execution.
            if (report_exceptions) {
                ReportException(isolate, &try_catch);
                __android_log_print(ANDROID_LOG_ERROR, "MyApp", ">>>>>>>>>>>>>>>>>>>>>ERR report_exceptions");
            }
            return false;
        } else {
            __android_log_print(ANDROID_LOG_ERROR, "MyApp", ">>>>>>>>>>>>>>>>>>>>>ERR Run ELSE");
            assert(!try_catch.HasCaught());
            if (print_result && !result->IsUndefined()) {
                __android_log_print(ANDROID_LOG_ERROR, "MyApp", ">>>>>>>>>>>>>>>>>>>>>ERR IsUndefined");
                // If all went well and the result wasn't undefined then print
                // the returned value.
                v8::String::Utf8Value str(isolate, result);
                const char *cstr = ToCString(str);
                __android_log_print(ANDROID_LOG_ERROR, "MyApp", ">>>>>>>>>>>>>>>>>>>>> %s\n", cstr);
            }
            return true;
        }
    }
...
void ReportException(v8::Isolate *isolate, v8::TryCatch *try_catch) {
    ...
    if (message.IsEmpty()) {
        ...
        if (try_catch->StackTrace(context).ToLocal(&stack_trace_string) &&
            ...
            __android_log_print(ANDROID_LOG_ERROR, "MyApp", ">>>>>>>>>>>>>>>>>>>>> stack_trace_string %s\n", stack_trace_string);
        }
    }
}